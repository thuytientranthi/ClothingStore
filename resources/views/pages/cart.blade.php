                       ...
                       ?>
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td class="cart_product_img d-flex align-items-center">
                                            <a href="#"><img src="{{asset($value['image'])}}" alt="Product"></a>
                                         
                                        </td>
                                        <td class="price"><span>{{ $value['describe']}}</span></td>
                                        <td class="price"><span> {{$value['price']}}</span></td>
                                    <form action='{{ action('UserControllers@updateCart',$value['id']) }}' method="post">
                                        @csrf
                                        <td class="qty">
                                            <div class="quantity">
                                                <!-- <span class="qty-minus" onclick="var effect = document.getElementById('qty'); var qty = effect.value; if( !isNaN( qty ) &amp;&amp; qty &gt; 1 ) effect.value--;return false;"><i class="fa fa-minus" aria-hidden="true"></i></span> -->
                                                <input type="number" class="qty-text" id="{{ $value['id'] }}" min="1" max="99" name="{{$value['id']}}" value="{{ $value['qty'] }}" />
                                                <!-- span class="qty-plus" onclick="var effect = document.getElementById('qty'); var qty = effect.value; if( !isNaN( qty )) effect.value++;return false;"><i class="fa fa-plus" aria-hidden="true"></i></span> -->
                                            </div>
                                        </td>
                                        <td class="total_price"><span>{{ number_format( $value['price']*$value['qty']).' VNĐ' }}</span>
                                        <td class="total_price"><span><a href="{{url("remove/{$value['id']}")}}">Xoa</a></span>
                        ...
                        <div class="cart-footer d-flex mt-30">
                            ...
                            <div class="update-checkout w-50 text-right">
                                <a href="{{url("delete")}}">clear cart</a>
                               <!--  <input type = "hidden" name="update" value = "updatecart" ></input> -->
